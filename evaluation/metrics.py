...
class RAGMetrics:
    ...
    def precision_at_k(self) -> float:
        ...
        relevant_and_retrieved = set(self.retrieved_tables) & \
                                  set(self.relevant_tables_ground_truth)
    ...
    def recall_at_k(self) -> float:
        ...
        relevant_and_retrieved = set(self.retrieved_tables) & \
                                  set(self.relevant_tables_ground_truth)
        return len(relevant_and_retrieved) / \
               len(self.relevant_tables_ground_truth)
    ...
    def print_metrics(self):
        ...
        print(f"  Retrieved (K) : {len(self.retrieved_tables)} "
              f"→ {self.retrieved_tables}")
        print(f"  Coverage ratio: {self.coverage_ratio:.1%} "
...
class SQLMetrics:
    ...
    def is_hallucinated(self) -> bool:
        return len(self.hallucinated_tables) > 0 or \
               len(self.hallucinated_columns) > 0
    ...
    def print_metrics(self):
        ...
        print(f"  Execution success : {'✅ Yes' if self.execution_success else '❌ No'}")
...
class FaithfulnessMetrics:
    ...
    def faithfulness_score(self) -> float:
        ...
        """
        if not self.insight or self.dataframe is None or \
        ...
        return len(matched) / len(numbers_in_insight) \
            if numbers_in_insight else 1.0
...
class ModelComparator:
    ...
    def print_comparison_table(self):
        ...
        print(f"{'=' * 80}")
        print(f"{'Model':<30} {'Avg Quality':>12} "
              f"{'Halluc Rate':>12} {'Avg Rows':>10} "
              f"{'Avg Time(ms)':>13} {'Success%':>10}")
        print("-" * 80)

        for model in sorted(models):
            model_entries = [e for e in self.entries
                             if e.model == model]
            avg_quality = sum(e.sql_quality for e in model_entries) / \
                          len(model_entries)
            avg_halluc = sum(e.hallucination_rate
                             for e in model_entries) / len(model_entries)
            avg_rows = sum(e.rows_returned
                           for e in model_entries) / len(model_entries)
            avg_time = sum(e.execution_time_ms
                           for e in model_entries) / len(model_entries)
            success_rate = sum(1 for e in model_entries
                               if e.success) / len(model_entries)

            print(f"{model:<30} {avg_quality:>11.1f} "
        ...
        if len(models) > 1:
            best = max(
                models,
                key=lambda m: sum(
                    e.sql_quality for e in self.entries if e.model == m
...
def check_column_hallucinations(sql: str, used_tables: list[str]) -> list[str]:
    ...
    col_matches = re.findall(
        r'(?:SELECT|WHERE|GROUP BY|ORDER BY|ON)\s+'
        r'([a-zA-Z_][a-zA-Z0-9_\.]*)',
        sql, re.IGNORECASE
    )

    keywords = {"distinct", "count", "sum", "avg", "max", "min",
                "as", "by", "from", "join", "on", "where",
                "group", "order", "limit", "having", "case",
...
def run_full_evaluation(
    ...
    # ── SQL Metrics ───────────────────────────────────────────
    used_tables, hallucinated_tables = extract_used_tables(
        sql_query, all_tables
    )
    hallucinated_cols = check_column_hallucinations(
        sql_query, used_tables
    )
    ...
    comparator.add(ModelComparisonEntry(
        model=model_name,
        question=question,
